...
parser.add_argument('--data', type=str, default='../data', help='temp data dir')
...
def main():
    ...
    unlabeled_queue = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size,
          sampler=torch.utils.data.sampler.SubsetRandomSampler(indices[:split]),
          pin_memory=True, num_workers=2)
    ...
    for sp in range(len(num_to_keep)):
        ...
        #optimizer_a = torch.optim.Adam(model.arch_parameters(),lr=args.arch_learning_rate, betas=(0.5, 0.999), weight_decay=args.arch_weight_decay)
...
def train(train_queue, valid_queue, unlabeled_queue, model, student, architect, criterion, criterion_stud, optimizer, optimizer_stud, lr, network_params, train_arch):
